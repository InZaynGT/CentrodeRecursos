...
if (isset($_POST['buscarDetalle']) && isset($_POST['idproveedorBus'])) {
	...
	if ($buscar == '') {
		...
		$detalle = $b->getInsumoCargaDescarga($buscarCodI[0]);
		$b = null;

		//Evaluamos si el detalle está vacío o no
		if (empty($detalle)) {
			sleep(1);
			echo '<div ><i class="fa fa-warning"></i> No se encontraron resultados.</div>';
		} else {
			echo '
				<div id="wrapper" >
			...
			echo '</b></td>
							<td width="20px" align="center"><b>Cantidad</b></td>
							<td width="20px" align="center"><b>Precio</b></td>
						</tr>
							';
			for ($i = 0; $i < count($detalle); $i++) {
				...
				$idDiv = new DateTime();
				echo ' <tr>
							   <td><input type="radio" name="radio_prod" value="' . $detalle[0]['idmedicamento'] . '" data-iddiv="' . $idDiv->getTimestamp() . '" data-codigo="' . $detalle[0]['idmedicamento'] . '" ' . $select . '></td>
								 <input type="hidden"  name="codprod" id="codprod" value="' . $detalle[0]['idmedicamento'] . '" min="1" required/>
								 <input type="hidden"  name="insumoNombre" id="insumoNombre" value="' . $detalle[0]['nombre'] . '" min="1" required/>
								 <input type="hidden"  name="existenciaInsumo" id="existenciaInsumo" value="' . $detalle[0]['existencia'] . '" required/>
							   <td>' . $detalle[0]['nombre'] . '</td> ';
				if ($_POST['mv'] != 3) {
					echo '
													  <td> <input type="number"  name="precio" step="0.01"  id="precio" value="' . $detalle[0]['precioP'] . '" min="1" required  onkeypress="enterSelProd(event)"/></td>
														';
				} else {

					echo '
 													  <td> <input type="number" step="0.01"  name="precio" id="precio" value="' . number_format($detalle[0]['precioP'], '2', '.', ',') . '" min="1" required  onkeypress="enterSelProd(event)"/></td>
 														';
				}
				echo '	 <td> <input type="number"  name="cantidad" id="cantidadp" value="1" min="1" step="0.01" required onkeypress="enterSelProd(event)"/></td>
										<td><input type="number" ';
				if (($_POST['mv'] == 2) || ($_POST['mv'] == 3)) {
					echo 'readonly';
				}
				echo ' name="precioV" step="0.01"	id="precioV" value="' . $detalle[0]['precioP'] . '" required/></td>
						   </tr>
							 <tr>
							 			<td></td>
										<td></td>
		 								 <td>Existencia:</td>
		 								 <td>' . $detalle[0]['existencia'] . '</td>
										 <td></td>
...
/////////////////////  GUARDAR   /////////////////////////////////////////////////////
	...
	if (isset($_POST['movimiento']) == 0) {
		...
		if (!isset($_POST['producto'])) {
			...
			// 	{
			// 		$t = new Proveedor();
			// 		//$saldoC = $t->getSaldoProveedor($idP);
			// 		$t = null;
			// 		$dc = $idMovimiento;
			// 		// $recibe = $_SESSION['user']['id_empleado'];
			// 		$recibe = $_SESSION['user']['username'];;


			// 		$saldo = $saldoC['SALDO'] + $totalMovimiento;
			// 		$cuentaCC = NULL;
			// 		$nc2 = new Proveedor(); //echo $idP.' '.$movimiento.' '.$tipoMov.' '.$cuenta.' '.$nCorrelativo.' '.$totalMovimiento.' '.$saldo.' '.$fecha_f.' '. $recibe;
			// 		//$nc2->setCuentaProveedor2($idP,$movimiento,$tipoMov,$cuentaCC,$nCorrelativo,$totalMovimiento,$saldo,$fecha_f, $recibe);
			// 		$nc2 = null;
			// 	}
			// }
			...
			// if ($movimiento == 2) 
			// {
			// 	if ($mensualidades == '' || $mensualidades == 1) 
			// 	{
			// 		echo '<div class="alert alert-danger alert-dismissable">
			// 													<button type="button" class="close" data-dismiss="alert" aria-hidden="true"><i class="fa fa-times-circle-o"></i></button>
			// 													<b>Error:</b> Ingrese campo Gasto en mensualidades. El valor debe ser mayor a 1.
			// 												</div>';
			// 		exit;
			// 	}
			// 	if ($mensualidades > 0) 
			// 	{
			// 		$totalGasto = $totalMovimiento / $mensualidades;
			// 		require_once 'models/gastoModel.php';
			// 		for ($g = 0; $g < $mensualidades; $g++) 
			// 		{
			// 			$actual = strtotime($_POST['fecha']);
			// 			$actual = date("Y-m-d", strtotime("+" . $g . " month", $actual));
			// 			$cuota = $g + 1;
			// 			$tipoG = 1;
			// 			$descripcion = 'Couta ' . $cuota . ' Salida No.' . $nCorrelativo;
			// 			$finca = 0;
			// 			$l = 1;
			// 			$factura = NULL;

			// 			// $ncb = new Gasto();
			// 			$ncb->setGasto($descripcion, $tipoG, $finca, $l, $actual, $totalGasto, $idC, $factura, $usuario, $usuario, '', '', '', 0, '', '', $_SESSION['id_centro']);
			// 			$ncb = null;
			// 		}
			// 	}
			// }

			//INSERTAR MOVIMIENTO EN ESTADO DE CUENTA SI EL PAGO FUE TRANSFERENCIA O DEPOSITO
			// if ($formaPagoV==6) {
			// 	require_once 'models/clienteBancoModel.php';
			// 	$ncb = new clienteBanco();
			// 	$saldoC=$ncb->getSaldo($cuentaBancaria);
			// 	$ncb=null;
			//
			// 	$saldo=$saldoC[0]['SALDO']+$totalMovimiento;
			// 	$tipo=1;  $mov=5; $descripcion='Venta '.$nCorrelativo; $esCheque=0;
			//
			// 					$ncb = new clienteBanco();
			// 					$ncb->setCardex($cuentaBancaria,$tipo,$mov,$descripcion, $totalMovimiento, $saldo,$fecha_f,$usuario,$documento, $nCorrelativo,$esCheque);
			// 					$ncb=null;
			// }
	...
	foreach ($insumos as $i) {
		$tags4 .= '"' . $i['idmedicamento'] . '- ' . $i['nombre'] . ' :: ' . $i['existencia'] . '",';
	}

	require_once 'views/header.php';
	require_once 'views/insumosCargaDescarga.php';
